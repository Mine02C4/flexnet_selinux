...
if [ $# -eq 1 ]; then
	if [ "$1" = "--update" ] ; then
		time=`ls -l --time-style="+%x %X" flexnet.te | awk '{ printf "%s %s", $6, $7 }'`
		rules=`ausearch --start $time -m avc --raw -se flexnet`
		if [ x"$rules" != "x" ] ; then
			...
			if [ "$ANS" = "y" -o "$ANS" = "Y" ] ; then
				...
				echo -e "$rules" | audit2allow -R >> flexnet.te
...
set -x
make -f /usr/share/selinux/devel/Makefile flexnet.pp || exit
